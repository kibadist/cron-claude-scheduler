...
3. **Run** — `claude -p --dangerously-skip-permissions` is spawned in a **disposable git worktree** of the tip of `baseBranch` (your main checkout is never touched — no branch switches, no dirty-tree collisions), with a prompt built from the ticket's title, description, and comments, plus the git-flow instructions. **Images pasted into the ticket** (screenshots, design mocks) are downloaded with your API key and handed to the agent as local files it actually looks at — both here and during verification. Output streams to `logs/<TICKET>-<timestamp>.log`. The worktree is removed after the run; only what was pushed survives.
